...
def deletar_curso():
    """Insere um novo curso na tabela tb_cursos."""
    st.subheader(":blue[Deletar curso]")
    nome_curso = st.text_input("Nome do Curso:")
    if st.button("Deletar Curso"):
        try:
            cursor.execute("DELETE FROM tb_cursos WHERE 1 = id_curso")
            conn.commit()
            st.toast("Curso deletado com sucesso!", icon="🆗")
            #st.success("Curso inserido com sucesso!")
        except sqlite3.IntegrityError:
            st.error("Erro: Já existe um curso com esse nome.")
...
# Menu lateral
opcoes = ["Inserir Curso", "Deletar Curso", "Inserir Aluno", "Exibir Dados"]
escolha = st.sidebar.selectbox("Selecione uma opção:", opcoes)

if escolha == "Inserir Curso":
    inserir_curso()
if escolha == "Deletar Curso":
    deletar_curso()
